...
const REDES: { chave: "instagram" | "linkedin" | "github"; nome: string; url: string; path: string }[] = [
  {
    chave: "instagram",
    nome: "Instagram",
...
export default function Autor({ t }: { t: Dicionario }) {
  const a = t.autor;
    ...
    <section id="autor" className="sec sec--tight sec--white">
      <div className="wrap">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 30, alignItems: "center" }}
          ...
          <div>
            <h2 style={{ fontSize: "clamp(30px,4.4vw,46px)", marginBottom: 14 }}>{a.nome}</h2>
            <p style={{ fontSize: 17, color: "var(--muted)", maxWidth: "62ch", margin: 0 }}>{a.bio1}</p>
            <p style={{ fontSize: 17, color: "var(--muted)", maxWidth: "62ch", marginTop: 12 }}>{a.bio2}</p>

            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 24 }}>
              {REDES.map((r) => (
                <a key={r.chave} href={r.url} target="_blank" rel="noopener noreferrer me author" className="mono"
                  style={{
                    display: "inline-flex", alignItems: "center", gap: 9,
                    padding: "12px 18px", fontSize: 12.5, fontWeight: 700, letterSpacing: ".08em",
                    textTransform: "uppercase", textDecoration: "none",
                    border: "2px solid var(--brand)",
                    background: r.chave === "instagram" ? "var(--brand)" : "transparent",
                    color: r.chave === "instagram" ? "#fff" : "var(--brand)",
                  }}>
          ...
          <div className="hide-sm" style={{
            ...
            {a.numeros.map((n) => (
              <div key={n.k} style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 9 }}>
                <span className="disp" style={{ fontSize: 26, color: "var(--brand)", minWidth: 62 }}>{n.n}</span>
                <span className="mono" style={{ fontSize: 11.5, color: "var(--muted)" }}>{n.k}</span>
              </div>
            ))}
          </div>
        </div>
