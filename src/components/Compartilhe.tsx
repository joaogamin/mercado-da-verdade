...
export default function Compartilhe({ t, locale }: { t: Dicionario; locale: Locale }) {
  const c = t.compartilhe;
  const url = urlDoLocale(SITE_URL, locale);
  const wa = `https://wa.me/?text=${encodeURIComponent(`${c.textoCompartilhado} ${url}`)}`;
  const x = `https://twitter.com/intent/tweet?text=${encodeURIComponent(c.textoCompartilhado)}&url=${encodeURIComponent(url)}`;

  const script = `(function(){
  var b=document.getElementById('copiar-link'); if(!b) return;
  b.addEventListener('click',function(){
    var url=${JSON.stringify(url)}, ok=${JSON.stringify(c.linkCopiado)};
    function feito(){ var o=b.textContent; b.textContent=ok; setTimeout(function(){b.textContent=o;},2000); }
  ...
  return (
    <section id="compartilhe" className="sec sec--tight" style={{ background: "var(--accent)" }}>
      <div className="wrap">
        <div className="eyebrow" style={{ color: "var(--ink)" }}>{c.eyebrow}</div>
        <h2 className="sec-title" style={{ color: "var(--ink)", maxWidth: "24ch" }}>{c.titulo}</h2>
        <p style={{ fontSize: 17, color: "var(--ink)", maxWidth: "60ch", marginTop: 16, opacity: 0.85 }}>{c.lead}</p>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 26 }}>
          ...
          <button id="copiar-link" type="button" style={botao}>{c.copiarLink}</button>
        </div>

        <p className="mono" style={{ fontSize: 12.5, color: "var(--ink)", opacity: 0.7, marginTop: 20 }}>{c.nota}</p>
      </div>
